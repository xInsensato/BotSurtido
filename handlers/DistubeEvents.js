          ...
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                ...
                } Me Desconecto Un Gil <#${voiceMap.get(
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setTitle(
                `Escribila bien culiado \`${queue?.songs[0].name}\``
              ),
          ],
        })
        .then((msg) => {
      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`Te escuchaste todo pa.`),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
      ...
      message.channel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`No puedo buscar mi loco \`${quary}\``),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(
                `${client.config.emoji.ERROR} No encontre nada \`${quary}\`!`
              ),
          ],
        })
        .then((msg) => {
        ...
        queue.textChannel
          .send({
            embeds: [
              new EmbedBuilder()
                .setColor(client.config.embed.color)
                .setDescription("Me dejaron re solo, manga de gatos"),
            ],
          })
          .then((msg) => {
            setTimeout(() => {
          ...
          case "autoplay":
            {
              if (!channel) {
                return send(
                  interaction,
                  `** ${client.config.emoji.ERROR} Metete en un canal virgo**`
                );
              } else if (
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} Metete en mi canal bb`
                );
              } else if (!queue) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} No toi reproduciendo nada  `
                );
              } else if (checkDJ) {
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} no tenes permisos ja`
                );
              } else if (!queue.autoplay) {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Autoplay Prendido `
                );
              } else {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Autoplay Apagado`
                );
              }
            }
            break;
          case "skip":
            {
              if (!channel) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} Tenes que entrar en un canal dawn`
                );
              } else if (
                interaction.guild.members.me.voice.channel &&
                !interaction.guild.members.me.voice.channel.equals(channel)
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} Metete en mi canal dale `
                );
              } else if (!queue) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} toi calladito `
                );
              } else if (checkDJ) {
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} e`
                );
              } else {
                await skip(queue);
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS}  Skipeaste la cancion mi loco`
                );
              }
            }
            break;
          case "stop":
            {
              if (!channel) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} Metet en un canal, dale te re cuesta`
                );
              } else if (
                interaction.guild.members.me.voice.channel &&
                !interaction.guild.members.me.voice.channel.equals(channel)
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} Ya me canse, metete a mi canal  `
                );
              } else if (!queue) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} modo nisman activado `
                );
              } else if (checkDJ) {
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} queseyo me aburro `
                );
              } else {
                await queue.stop().catch((e) => {});
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Anda cagar gil`
                );
              }
            }
            break;
          case "pauseresume":
            {
              if (!channel) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} Tenes que estar en un canal loco, cuantas veces te lo digo?`
                );
              } else if (
                interaction.guild.members.me.voice.channel &&
                !interaction.guild.members.me.voice.channel.equals(channel)
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} sos tontito? `
                );
              } else if (!queue) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} mas callado que la mierda `
                );
              } else if (checkDJ) {
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} que paja poner una linea de texto`
                );
              } else if (queue.paused) {
                await queue.resume();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Sigue la Cola`
                );
              } else if (!queue.paused) {
                await queue.pause();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Pausaste la cola boludo `
                );
              }
            }
            break;
      ...
      if (message.channelId === musicchannel.id) {
        ...
        if (message.author.bot) {
          ...
          if (
            ...
            return client.embed(
              message,
              `** ${client.config.emoji.ERROR} Dame Permisos dawnm, no puedo entrar**`
            );
          } else if (!voiceChannel) {
            return client.embed(
              message,
              `** ${client.config.emoji.ERROR} Metete en un canal putamadre **`
            );
          } else if (
            ...
            return client.embed(
              message,
              `** ${client.config.emoji.ERROR} queseyo rebuscatela **`
            );
          } else {
